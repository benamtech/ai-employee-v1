...
| `mvp-build/` | ★ **Where the AMTECH AI Employee MVP gets built** — the only root folder beside `wiki/`. Current reconciled build plan = `wiki/MVP/build-plan-current/`; **current second-half forward plan = `mvp-build/second-half-plan/`**; original build packet = `wiki/MVP/old-build-plan/`; product mechanics = `wiki/product-ai-employee-context.md`. The backend now has meaningful Hermes/Manager/runtime/event seams, but owner web/SMS surfaces are not trial-grade. The second-half plan reframes the next era around a Hermes-backed small-business materialization layer: `SurfaceEnvelope`, `WorkResource`, `WorkAction`, `EmployeeEventStream`, generic renderers, SMS signed previews, serious web employee desk, admin/factory operations, billing, and free-trial readiness. Existing source includes profile-package rendering, Manager tools, Manager-as-MCP, artifacts/approvals, Gmail/Stripe/event seams, runtime health, turn queue, channel router, metering ledgers, and local live-test tooling; provider/runtime acceptance and real owner-surface polish remain pending. Immediate next work: complete `mvp-build/second-half-plan/phase-01-preserve-and-close-live-gate.md` using `phase-01-handoff-prompt.md`, then proceed to the web/SMS materialization phases. **`mvp-build/` is tracked in the root `GTM-RESEARCH` git repo** (branch `main`; GitHub remote `origin` → `benamtech/ai-employee-v1`) with a build-home agent guide (`mvp-build/CLAUDE.md` + mirror `mvp-build/AGENTS.md`) and in-repo durable memory (`mvp-build/memory/`). | n/a |
