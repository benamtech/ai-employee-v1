...
>
> Generated: 2026-06-27 (last updated 2026-06-30, **current MVP build plan = `wiki/MVP/build-plan-current/`; original build packet = `wiki/MVP/old-build-plan/`; build home = `mvp-build/`; MVP source map = `mvp-build/CODEGRAPH.md`; implementation records = `wiki/MVP/implementation-records/`; production docs include admin + metering plans in `mvp-build/docs/`** — the AI Employee MVP build packet, prompting guide, implementation handoffs, research addenda, and implementation records under `wiki/MVP/` are the source-of-truth family, and the MVP is built in the root folder `mvp-build/`. The earlier `AI_EMPLOYEE_MVP/` prototype (which proved the mechanics and partly inspired the `wiki/MVP/build-plan` style) has been **REMOVED from the repo**; its mechanics live on in the wiki + ledger S081–S085. The root now holds exactly two folders: `wiki/` and `mvp-build/`, plus root control docs and the root workspace explorer `index.html`) · Corpus: 62 markdown wiki pages + 1 root HTML explorer + 2 root docs + the `mvp-build/` build home.
...
| **Implementation records** | Factual companion packet to the original build plan. Current records: Phase 0/1/2 wired; Phase 3 Gmail/work-event source wired; Phase 4 Stripe test-mode source wired; **Phase 5 close-the-loop wired**; **Phase 6 repair/security/runtime hardening + Phase 7 event-bus seams wired** (`0007` repair/jobs migration, repair tools, source suppression, triage/batching, `deliver_only` vs `wake_employee`, generic event-source registry, stored `[SILENT]` daily briefs, binding work-card approvals, Work Surface SSE seam); **new-era Phase 2 runtime/scheduler productionization source-wired** (`0008` runtime health migration, Docker-default backend, scheduler runner, `hermes_job_runs`, `runtime_health_checks`); **25 files / 124 passing local unit tests** (+ env-gated integration); provider/runtime acceptance remains pending. Records use CODEGRAPH for orientation and do not rewrite the original build-plan packet. | `wiki/MVP/implementation-records/README.md`, `wiki/MVP/implementation-records/2026-06-29-phase-0-2-record.md`, `wiki/MVP/implementation-records/2026-06-29-phase-3-partial-record.md`, `wiki/MVP/implementation-records/2026-06-29-phase-5-and-work-surface-record.md`, `wiki/MVP/implementation-records/2026-06-30-phase-6-and-event-bus-record.md`, `wiki/MVP/implementation-records/2026-06-30-phase-02-runtime-scheduler-record.md` |
...
| `mvp-build/` | ★ **Where the AMTECH AI Employee MVP gets built** — the only root folder beside `wiki/`. Current reconciled build plan = `wiki/MVP/build-plan-current/`; original build packet = `wiki/MVP/old-build-plan/`; product mechanics = `wiki/product-ai-employee-context.md`. **Phase 3 Gmail/work-events, Phase 4 Stripe test-mode, Phase 5 close-the-loop, Phase 6/7 production/event-bus seams, and new-era Phase 2 runtime/scheduler productionization are source-wired; provider/runtime acceptance pending:** TS/Node npm-workspaces monorepo — `apps/{web,manager}`, `packages/{shared,db,agent-template}`, `infra/`, `tests/`; Phase 0 schema/security/tool seams; real Phase 1 Manager identity/provisioning/brain tools, LLM-only web/SMS front-door orchestrator, single-use SMS claim links, owner web session, production-shaped HTTP provisioner, vertical-agnostic profile-package model with first package `contractor_estimator`, deterministic post-claim profile-package rendering/validation/runtime hooks (**not** prompt-to-repo authoring after claim), first-live-SMS proof seam, and owner web/SMS message routing to runtime endpoint; old Phase 2 Manager artifact/approval tools, private Supabase Storage artifact handling, signed artifact route, owner artifact/approval UI, and employee template instructions for Manager-backed estimate artifact flow; Phase 3 source includes Gmail OAuth/token/send/watch/history/PubSub, JWKS-authenticated push verification, typed work-event descriptors, inbound reply normalization, owner-phone SMS delivery lookup, and web Work Surface resource rendering; Phase 4 source includes Stripe Connect account creation, account links, onboarding status, deposit invoice create/send, approval gates, signed webhook processing, idempotency, and paid-invoice work events; Phase 5 source includes owner-confirmed internal reminders, `dispatch_due_reminders`, `renew_expiring_watches`, and stored `[SILENT]` daily briefs; new-era Phase 2 source includes Docker-default runtime backend policy, profile backend rendering, protected `/manager/scheduler/run`, `hermes_job_runs` proof writes, `runtime_health_checks`, `scheduler:hermes-jobs`, and `scheduler:tick` as the dev/manual fallback; Phase 6/7 source includes `0007` repair/jobs migration, repair tools, safe audit redaction, source suppression, triage/batching seams, binding work-card approvals, `deliver_only` vs `wake_employee`, generic event-source registry, structured Hermes event calls, and a Work Surface SSE seam. Local checks pass (`typecheck`, 124 unit tests across 25 files + env-gated integration skip, `build`, `lint`), but live Supabase/Twilio/Hermes/Gmail/PubSub/Stripe proof and real Docker/Hermes Jobs runtime proof are still pending. Stack: hosted Supabase (PG+Auth+Storage), single VPS or tunnel-exposed host (Hermes+Manager+Caddy+Next.js + Docker-contained employee runtimes), Twilio Verify + 10DLC. Replaces the earlier `AI_EMPLOYEE_MVP/` prototype (**removed 2026-06-28**; mechanics preserved in the wiki + ledger S081–S085). The root `index.html` explorer now seeds selected `mvp-build/` docs/source files and can load the full tree through the folder picker. **`mvp-build/` is now a local-only git repo** (branch `main`, no remotes) with a build-home agent guide (`mvp-build/CLAUDE.md` + mirror `mvp-build/AGENTS.md`) and **in-repo durable memory** (`mvp-build/memory/` + its writing protocol); the forward plan is `wiki/MVP/build-plan-current/phases/`, the **Phase 1 live-acceptance harness is built and locally verified** (gate pending live creds), and **Phase 2 runtime/scheduler productionization is source-wired** (runtime gate pending real Docker/Hermes Jobs proof). | n/a |
| `mvp-build/CODEGRAPH.md` | ★ MVP-specific source map for implementation agents. Explains that `mvp-build/` is the AI Employee product/provisioning/runtime home inside the larger AMTECH brain; maps end-user flows to source files; documents the Hermes/Nous substrate boundary, Manager-owned product layer, working/source-wired features, pending provider/runtime acceptance, and the current session-management priority around the Channel/Session/Presence router. | active |
...
| `mvp-build/memory/` | ★ **In-repo durable memory** for agentic development — dated dev handoffs + the memory writing protocol (`MEMORY.md`): write/update mid-session after substantial/architectural work, after a full phase, and after architectural decisions. Dated handoffs named `YYYY-MM-DD-HHMM-title.md`; current: `2026-06-30-0120-rephase-and-phase-1-acceptance.md`. Complements `wiki/MVP/implementation-records/` (factual code-state). | active |
...
```json
{
  ...
  "mvpBuildCodegraph": "mvp-build/CODEGRAPH.md (MVP-specific source map for implementation agents: end-user flows to files, Hermes/Nous substrate boundary, Manager-owned product layer, working/source-wired features, pending acceptance, and Channel/Session/Presence priority)",
  "mvpBuildHome": "mvp-build/ (the only root folder beside wiki/; where the MVP is built against wiki/MVP/build-plan-current/ with wiki/MVP/old-build-plan/ preserved for original mechanics. Phase 3 Gmail/work-events, Phase 4 Stripe test-mode, and Phase 5 close-the-loop are source-wired plus a descriptor-driven Work Surface redesign, provider/runtime acceptance pending: TS/Node npm-workspaces monorepo — apps/{web,manager}, packages/{shared,db,agent-template}, infra/, tests/; schema/security/tool seams plus real identity/provisioning/brain tools, LLM-only front door, SMS claim links, production-shaped provisioner, vertical-agnostic profile packages with contractor_estimator first, deterministic post-claim profile-package rendering/validation/runtime hooks (not prompt-to-repo authoring after claim), first-live-SMS proof seam, owner web/SMS runtime routing, Manager-backed estimate artifact storage/link/approval flow, Gmail OAuth/token/send/watch/history/PubSub/event seams, typed work-event descriptors, Stripe Connect/deposit invoice tools, Phase 5 owner-confirmed reminders + dispatch_due_reminders/renew_expiring_watches + scheduler:tick seam, the Work Surface (daily brief/job folders/notify-question-review cards/per-deliverable renderers/iterative feedback), Phase 6/7 production/event-bus seams, admin/metering docs, and a passing typecheck/unit/build/lint suite)",
  ...
  "nodes": [
    ...
    {"id": "wiki/MVP/build-plan-current/phases/phase-03a-channel-session-presence-layer.md", "type": "mvp-phase", "status": "planned"},
