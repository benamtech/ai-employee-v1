...
>
> Generated: 2026-06-27 (last updated 2026-07-14, **current MVP build plan = `wiki/MVP/build-plan-current/`; current second-half forward plan = `mvp-build/second-half-plan/`; master UX system = `mvp-build/docs/ux/`; active owner UI packet = `mvp-build/ui-redesign/`; current second-half wiki companion = `wiki/MVP/second-half-current-and-future-state.md`; original build packet = `wiki/MVP/old-build-plan/`; build home = `mvp-build/`; MVP source map = `mvp-build/CODEGRAPH.md`; implementation records = `wiki/MVP/implementation-records/`; production docs include admin + metering + UX plans plus the 2026-07-11 production-deploy-readiness review in `mvp-build/docs/`, and the re-sequenced production-runtime-and-deploy roadmap in `mvp-build/second-half-plan/`** — the AI Employee MVP build packet, prompting guide, implementation handoffs, research addenda, and implementation records under `wiki/MVP/` are the source-of-truth family, and the MVP is built in the root folder `mvp-build/`. The earlier `AI_EMPLOYEE_MVP/` prototype (which proved the mechanics and partly inspired the `wiki/MVP/build-plan` style) has been **REMOVED from the repo**; its mechanics live on in the wiki + ledger S081–S085. The root now holds exactly two folders: `wiki/` and `mvp-build/`, plus root control docs and the root workspace explorer `index.html`) · Corpus: 64+ markdown wiki pages + 1 root HTML explorer + 2 root docs + the `mvp-build/` build home.
...
| `mvp-build/docs/ux/` | ★ Master UX system for current and post-release AMTECH UI work: Aqua HIG translation, current UX source map, research ledger, implementation coverage audit, generative UI frontier, fixture-vs-production policy, and post-release roadmap. | active |
