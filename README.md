...
Current state (2026-07-11): the second-half plan in `mvp-build/second-half-plan/` is the active forward plan, and its product surfaces are `source-wired` — built and green against the local test suites (typecheck/unit/build/lint), i.e. operational within the test harness, not vaporware: the web Work Surface (Phase 2), the SMS ambient inbox + signed mobile Review page (Phase 3), the tool-agnostic materialization/capability layer with a generic Connector Center and a resurfacing projection (Phase 4), Gmail/Stripe/**QuickBooks** connectors, MCP-UI generative cards, and an internal operator admin console (Phase 5). What remains before real owners is **operations and live proof**, not more features: a production deploy/runtime layer that actually runs the fleet on a VPS (service supervision, employee-container launch, Caddy reload, backups, observability, egress), and live provider/runtime acceptance with real proof IDs. The deploy gap and the re-sequencing are in `mvp-build/docs/production-deploy-readiness-review-2026-07-11.md` and `mvp-build/second-half-plan/production-runtime-and-deploy-roadmap-2026-07-11.md`; admin-panel polish and billing are deliberately **parked** behind them.
...
The current major product/software priority is **production deployability and live proof of the core loop**: getting the box deployable and self-sustaining (service supervision, employee-container launch, Caddy reload, reboot recovery) and proving the source-wired owner → tool → artifact/approval loop against a live model/runtime, before any further admin or billing work. The session model — one employee, one number, one continuous thread across SMS/web/future voice, with a Manager-owned Channel/Session/Presence router deciding where output lands — is already source-wired.
