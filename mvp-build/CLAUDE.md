...
- **Phase 5 triage/batching + live Work Surface + MCP-UI:** `source-wired` (2026-07-04). Rules-first
  ...
  + `employee-stream.ts`) woken by `progress-bus.ts` (testable Realtime/NOTIFY stand-in) with cursor
  catch-up + poll fallback; Hermes `/v1/runs/{id}/events` streaming → owner-safe work-verbs; **generative
  UI via MCP-UI** — agent `view` → Manager-compiled `ui://` `rawHtml` `UIResource` (`ui-resources.ts`,
  `@mcp-ui/server`) in a sandboxed iframe whose `postMessage` intents route through the approval gate
  (`McpUiResource.tsx`). Live Hermes SSE + Supabase `0016` RLS proof pending.
...
Local truth: typecheck/build/lint pass; **44 unit files / 254 tests** pass; integration skips clean
