...
- **Production normal employee launch runbook + onboarding logging (2026-07-16):** `source-wired`; provider/live proof remains `provider-gated`. The canonical runbook for launch-level normal employee testing is now `docs/production-normal-employee-live-deploy-runbook.md`, with companion review `docs/onboarding-live-review-2026-07-16.md` and durable memory `memory/2026-07-16-0335-production-normal-employee-runbook-default-handoff.md`. It makes public DNS/Cloudflare Tunnel -> Caddy -> real onboarding/account/provisioning -> provider-backed owner-client reply the default proof path, and explicitly excludes fixture/local/dev-login/public-estimator harnesses from launch acceptance. Manager onboarding now classifies provider auth/credit failures as `provider_auth_or_credit_gated` and logs sanitized structured onboarding model events; the 2026-07-16 live attempt reached public ingress through Cloudflare/Caddy but xAI returned a team credit/monthly-spending-limit block, so no employee runtime or launch proof was created.
