...
Hermes context-engineering plan (agent brain, native memory, hooks, session rotation) lives in
`second-half-plan/context-engineering/` (planned; docs-only).
...
- **Context-engineering workstream (2026-07-12):** `planned` (docs-only; no code). New parallel workstream `second-half-plan/context-engineering/` (README + `phase-ce-01..04`) making each employee a real business brain, grounded in deep Hermes-substrate research. Core reframe: the "business brain" is the whole integrated employee system (Hermes + multi-connector event ingress + turns + Manager + onboarding + learned state, assembled in `lib/employee-stream.ts`), not a facts stub; and never inject a per-turn digest — prime with references once per session. CE-1 seeds Hermes-native `MEMORY.md`/`USER.md` from onboarding + a cache-safe `pre_llm_call` live-state primer (`buildAgentContext`) + fixes the render/data drops (`profile-renderer.ts` PRICING/BRANDING/workflows, `get_business_brain` returning only a count); CE-2 renders a tuned `compression:` block + wires `hooks:`/transform hooks + moves trivial/background work off the owner turn (`deliver_only` + delegation/Jobs); CE-3 rotates sessions before compaction with a handoff carryover (keeps `X-Hermes-Session-Key`); CE-4 (deferred) connector-agnostic capabilities (Manager-mediated for money/customer-facing MCP connectors) + operator modes. See `memory/2026-07-12-0030-context-engineering-workstream-planned.md`.
