...
> **2026-07-16 launch-run note:** For normal employee live deploy work, start
> with [`docs/production-normal-employee-live-deploy-runbook.md`](docs/production-normal-employee-live-deploy-runbook.md).
> The accepted launch path is public DNS/Cloudflare Tunnel -> Caddy -> Web/Manager
> -> real `/create-ai-employee` -> Twilio Verify -> account creation -> Start
...
> onboarding, Twilio Verify, and account creation.
...
9. For live normal-employee deploy/testing: `docs/production-normal-employee-live-deploy-runbook.md`.
10. This `CODEGRAPH.md`, then the relevant authored source files below.
...
- **Second-half plan (2026-07-09):** active in `second-half-plan/`. The backend is ahead of the owner product; the next seven phases are: Phase 0 handoff, Phase 1 preserve/close live gate, Phase 2 Avery-first owner UI, Phase 3 SMS ambient inbox/signed previews, Phase 4 tool-agnostic capability/rendering/materialization, Phase 5 trial ops/admin/billing, Phase 6 free-trial/paid-pilot readiness.
...
- **Second-half Phase 2 owner Work Surface redesign:** `source-wired` with local fixture/headed UI proof. The owner web surface has been reset to the active Avery-first MVP direction: Home / Talk / Proof / Connected, Tell Avery, inline Needs your say exact review, quiet Watching, Recent proof, optional work sheet, persisted conversation, SSE/poll fallback, approval/work-card reuse, and generic artifact HTML fallback. The older multi-region employee desk and rejected chat-native agent desktop are historical context only. Live provider/runtime acceptance remains pending.
...
- **Current priority (live-testing phase):** use the Avery-first owner UI as the current owner product target, then apply live migrations/proofs when env/approval allows; seed a real platform operator row for the admin surface; reprovision fresh employees to prove scoped MCP auth and context-engineering hooks; close the live tool-execution gate with a real Hermes model loop / funded provider, then provider-prove Phase 3 SMS previews and Phase 4/5 materialization/admin readiness against live Hermes/provider events. **QuickBooks live gate:** register an Intuit developer app, fill the `QBO_*` env, and run the sandbox handshake → approved `create_expense` → `commit_quickbooks_write` → webhook path (verifier `run10-quickbooks.mjs`); **re-verify Intuit's CloudEvents webhook envelope + refresh-token rotation policy against live docs before relying on them**. Egress control remains a pre-tenant requirement.
...
- **Context-engineering CE-4 + estimator materialization research (2026-07-12):** `source-wired` for CE-4; live direct-MCP connector proof still `pending`. Added `packages/shared/src/connector-registry.ts` as the declarative connector metadata/custody source. `capability-registry.ts` now routes connector-backed statuses by connector category/metadata instead of tool-name substring branches and projects direct read-only MCP connector nodes generically; `employee-stream.ts` uses the same custody metadata for Connected surface copy. `profile-renderer.ts` renders `direct_mcp_connectors` into `config.yaml` only when custody is read-only; write/money/customer-facing requests are dropped and remain Manager-mediated. `agent-context.ts` exposes a business-type/operator-mode primer policy seam; `session-rotation.ts` accepts an optional policy rotate ratio. Research doc `second-half-plan/estimator-product-limits-research.md` studies the conversational website estimator materialization: one generated employee, many visitor conversations, local website chat as the form, artifact-backed HTML/PDF output. Key finding: the reusable pieces exist, but visitor-session isolation and deterministic HTML-to-PDF are the two clearest missing primitives. The current declarative cross-plan update is `docs/state-of-progress-2026-07-14.md`.
- **Production normal employee launch runbook + onboarding/provisioning live run (2026-07-16):** `source-wired`; full launch proof remains `pending`. The canonical runbook for launch-level normal employee testing is `docs/production-normal-employee-live-deploy-runbook.md`, with companion review `docs/onboarding-live-review-2026-07-16.md` and latest handoff `memory/2026-07-16-0538-provisioner-failure-live-production-handoff.md`. Public DNS/Cloudflare Tunnel -> Caddy -> real onboarding/account/provisioning -> provider-backed owner-client reply is the default proof path; fixture/local/dev-login/public-estimator harnesses are excluded from launch acceptance. xAI/Grok `grok-4.3` is currently reachable and public onboarding now uses a proper stream-then-extract SSE path. A live run reached public onboarding, real Twilio Verify, real account creation, and Start Employee, but `provision_employee` failed (`pjob_t934ptxbf0y8ar427q7s7u`, attempted `emp_t596x0vbwtd23xdqonfp09`). Root-cause work found provisioner failure logs are swallowed, malformed `profile_context.memory_limits` can crash rendering, and failed idempotency keys need a retry path before the full employee + tool-connection proof can complete.
...
Hermes/Manager/provider events
  ...
  -> Avery-first owner UI / SMS ambient inbox / signed preview / admin inspector / optional desktop
```
...
| `apps/web/app/_lib/ui-fixtures.ts` | Central fixture-mode policy for web surfaces. Allows fixture data only for local UI proof and fails closed when `NEXT_PUBLIC_AMTECH_UI_FIXTURES=1` appears in production-like or pod-like environments. |
...
| `apps/web/app/agent/[employeeId]/AgentClient.tsx` | Main owner MVP client: Avery-first Home / Talk / Proof / Connected with Tell Avery, inline Needs your say exact review, quiet Watching, Recent proof, optional work sheet, persisted conversation, SSE/poll fallback, and approval/work-card actions. |
...
| `infra/scripts/prod-env-proof.mjs` | Aggregates latest `infra/proofs/*.json` into a production-environment proof with explicit proof tier boundaries; fails production-like proof if UI fixture mode is enabled. |
...
| `docs/ux/` | ★ Master UX system for the current and post-release UI work: Aqua-to-AI interface principles, current surface map, research ledger, implementation coverage audit, generative UI frontier, fixture-vs-production policy, and post-release roadmap. Read before broad UI work; use `ui-redesign/` for the active owner MVP packet. |
...
| `second-half-plan/phase-02-owner-work-surface-redesign.md` | Historical Phase 2 owner-surface redesign plan. Superseded in product direction by the active Avery-first `ui-redesign/` packet and implementation. |
