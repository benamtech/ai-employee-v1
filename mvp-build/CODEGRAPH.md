...
| `apps/manager/src/lib/admin.ts` | Phase 5 Manager-only admin/ops read and action layer: platform-role enforcement, support-access audit, redacted dashboard/account/employee views, readiness reports, and scoped support actions including employee lifecycle and MCP credential rotation/revocation. |
...
| `apps/manager/src/lib/qbo-tokens.ts` | Sole owner of QBO token refresh: `intuit-oauth` for the actual refresh, sealed by reference (`secrets.ts`), guarded by an atomic per-connector `token_refresh_lease_until` compare-and-swap so concurrent requests never race the rotating refresh token. |
...
| `apps/manager/src/lib/qbo-lookup.ts` | TTL-cached per-connector name→id resolution (Customer/Vendor/Account/Item/Class/Department). Exact match resolves; ambiguous/zero returns `needs_disambiguation`/`not_found` — never a best-guess pick. |
