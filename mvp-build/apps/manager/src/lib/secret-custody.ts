...
export function validateSecretDescriptor(input: ManagedSecretDescriptor, now = Date.now()): ManagedSecretDescriptor {
  ...
  if (input.revoked_at && iso(input.revoked_at, "revoked_at") > now + 5 * 60_000) throw new Error("secret_revocation_future_invalid");
