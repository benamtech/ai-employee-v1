...
/** UI Lab changes fixture timing only; AgentSurface owns production compilation and rendering. */
export function ProductionFixtureLabClient({ scenarioId, employeeId }: { scenarioId: FixtureScenarioId; employeeId: string }) {
  ...
  const [notice, setNotice] = useState("Fixture truth; production compiler, planner, registry, WorkResource renderer, and embedded-view host.");
  ...
  function reset() {
    ...
    setInput("");
    setRunning(false);
    setNotice("Scenario reset from its deterministic snapshot. No command was replayed.");
    scheduleFrames(session.frames);
  }
  function gap() {
    clearTimers();
  ...
  function submit() {
    ...
    setInput("");
    setRunning(true);
    setNotice("Fixture intent accepted locally. Only the production projection path renders it.");
  ...
  return <main className="production-fixture-lab">
    <style>{CSS}</style>
    <header><div><Link href="/ui-lab">AMTECH<span>.</span></Link><strong>UI Lab</strong><small>same truth · same authority · same actions · presentation policy only</small></div><button onClick={reset}>Reset scenario</button></header>
    ...
    <section className="lab-summary"><div><p>Fixture demonstration only</p><h1>{session.scenario.label}</h1><span>{session.scenario.summary}</span></div><dl><div><dt>Evidence</dt><dd>fixture_demonstration</dd></div><div><dt>Compiler</dt><dd>production</dd></div><div><dt>Renderer</dt><dd>production</dd></div></dl></section>
    ...
    <section className="production-projection"><AgentSurface employeeId={employeeId} fixtureMode fixturePayload={payload} embedded /></section>
...
const CSS = `.production-fixture-lab{min-height:100vh;background:#f4f7fb;color:#10131a;padding-bottom:48px}.production-fixture-lab>header{position:sticky;top:0;z-index:70;display:flex;justify-content:space-between;gap:16px;padding:12px 4vw;background:rgba(255,255,255,.94);border-bottom:1px solid #dbe2eb}.production-fixture-lab>header>div{display:flex;align-items:center;gap:12px}.production-fixture-lab header a{font-weight:900;color:#111;text-decoration:none}.production-fixture-lab header a span{color:#e11d2a}.production-fixture-lab header small{color:#667085}.production-fixture-lab button{min-height:44px;padding:0 15px;border:1px solid #cdd5df;border-radius:999px;background:#fff;font-weight:750}.production-fixture-lab>nav{display:flex;gap:8px;overflow:auto;padding:14px 4vw}.production-fixture-lab>nav a{padding:8px 12px;border:1px solid #d6dde7;border-radius:999px;color:#344054;text-decoration:none;white-space:nowrap}.production-fixture-lab>nav a.active{background:#111;color:#fff}.lab-summary,.lab-heartbeat,.notice,.lab-command{width:min(1240px,calc(100% - 32px));margin:16px auto;padding:20px;border:1px solid #dbe2eb;border-radius:20px;background:#fff}.lab-summary{display:flex;justify-content:space-between;gap:24px}.lab-summary h1{font-size:clamp(30px,5vw,54px);margin:6px 0}.lab-summary p,.lab-heartbeat p{font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:#667085;font-weight:800}.lab-summary dl{display:grid;gap:7px}.lab-summary dl div{padding:8px 10px;border:1px solid #e4e9f0;border-radius:10px}.lab-summary dt{font-size:10px;color:#667085}.lab-summary dd{margin:2px 0 0;font-size:12px;font-weight:700}.lab-heartbeat{display:grid;gap:10px}.lab-heartbeat.stalled{border-color:#fda29b}.lab-heartbeat .actions{display:flex;gap:8px;flex-wrap:wrap}.notice{background:#eef4ff;color:#175cd3;font-size:13px}.production-projection{border-block:1px solid #dbe2eb;background:#f8fafc}.lab-command{display:grid;gap:10px}.lab-command textarea{padding:12px;border:1px solid #cdd5df;border-radius:12px;font:inherit}.lab-command button{justify-self:start;background:#e11d2a;color:#fff;border-color:#e11d2a}@media(max-width:720px){.lab-summary{display:grid}.production-fixture-lab>header>div{align-items:flex-start;flex-wrap:wrap}}`;
