...
def build_candidate_graph():
    ...
    edges.extend(CG["higher_order_edges"])

    incidence = np.zeros((len(IDS), len(edges)), dtype=float)
    for edge_index, edge in enumerate(edges):
        assert edge["members"], edge["name"]
        assert all(member in INDEX for member in edge["members"]), edge["name"]
    ...
    laplacian = (
        np.eye(len(IDS))
        - np.diag(1.0 / np.sqrt(np.maximum(vertex_degree, 1e-12)))
        @ incidence
    ...
    distance = np.sqrt(((score_matrix[:, None, :] - score_matrix[None, :, :]) ** 2).sum(axis=2))
...
METRIC_CACHE: dict[tuple[str, ...], dict[str, float | list[str]]] = {}
...
def feasible(selection) -> bool:
    ...
    if len(selected) != SET_SIZE or len(set(selected)) != SET_SIZE:
        return False
    if not set(selected).issubset(VALIDATED):
        return False
    workstreams = {
        workstream
...
def software_coverage(selection) -> dict[str, float | list[str]]:
    represented: set[str] = set()
    concept_map = SG["concept_representations"]
    for candidate_id in canonical(selection):
        concept = CANDIDATES[candidate_id]["concept"]
        assert concept in concept_map, concept
        represented.update(concept_map[concept])
...
def metrics(selection) -> dict[str, float | list[str]]:
    selected = canonical(selection)
    ...
    vne = float(-np.sum(eigenvalues * np.log(eigenvalues)) / math.log(len(indices)))

    candidate_edge_touch = sum(
    ...
    output: dict[str, float | list[str]] = {
        "q_bar": float(mean(float(SCORE_ROWS[candidate_id]["q"]) for candidate_id in selected)),
        "candidate_edge_touch": float(candidate_edge_touch),
        "separation_min": min(distance_pairs),
        "separation_mean": float(mean(distance_pairs)),
        "von_neumann_entropy": vne,
        "quality_diversity_occupancy": len(selected_cells) / len(all_cells),
        "redundancy": float(mean(similarity_off_diagonal)),
    }
...
DIVERSITY_TERMS = {
    "separation_mean",
    "von_neumann_entropy",
    "quality_diversity_occupancy",
    "redundancy",
}


def evidence_baseline(selection) -> float:
    positive = ["Evidence", "BusinessValue", "VerificationFeasibility", "ProofDensity", "RiskReduction"]
    negative = ["Unsupported", "Cost", "Scope", "PrerequisiteDebt"]
    selected = canonical(selection)
    positive_mean = mean(
        mean(float(SCORE_ROWS[candidate_id]["r"][dimension]) for dimension in positive)
        for candidate_id in selected
    )
    negative_mean = mean(
        mean(float(SCORE_ROWS[candidate_id]["r"][dimension]) for dimension in negative)
        for candidate_id in selected
    )
    return float(positive_mean - negative_mean)
...
def objective(selection, mode: str, override: dict[str, float] | None = None) -> float:
    ...
    if mode == "no_graph":
        removed = GRAPH_TERMS
    elif mode == "no_diversity":
        removed = DIVERSITY_TERMS
    elif mode != "full":
...
def feasible_walk(start, seed: int, steps: int = 128) -> tuple[str, ...]:
    ...
    for _ in range(steps):
        outgoing = rng.choice(sorted(current))
        incoming = rng.choice(sorted(VALIDATED - current))
...
def local_optimize(
    ...
    for _ in range(rounds):
        swaps = [
            ...
            for incoming in sorted(VALIDATED - set(current))
        ]
        rng.shuffle(swaps)
        best = current
        ...
        for outgoing, incoming in swaps[:swaps_per_round]:
            ...
            if not feasible(candidate):
                continue
            candidate_score = objective(candidate, mode, override)
            if candidate_score > best_score + 1e-12 or (
                math.isclose(candidate_score, best_score, abs_tol=1e-12) and candidate < best
            ):
                best = candidate
...
def run_search(mode: str, starts: list[tuple[str, ...]], seed_offset: int, override=None):
    solutions = [
        local_optimize(start, mode, seed_offset + index, override)
        for index, start in enumerate(starts)
    ]
    solutions.sort(key=lambda item: (-item[1], item[0]))
    best_set, best_score = solutions[0]
    unique_sets = {selection for selection, _ in solutions}
...
def main() -> None:
    ...
    base_weights = {
        key: float(value)
        for key, value in CMP["objectives"]["full"].items()
    }
    ...
    for run_index in range(int(CMP["search"]["weight_sensitivity_runs"])):
        ...
        varied = {
            key: value * (1.0 + rng.uniform(-perturbation, perturbation))
            for key, value in base_weights.items()
        }
        sensitivity_starts = [full_set] + [
            feasible_walk(full_set, 91000 + run_index * 10 + offset)
    ...
    report = {
        ...
        "random_feasible_baseline": {
            ...
            "full_percentile": (
                sum(value <= searches["full"]["objective"] for value in random_values)
                / len(random_values)
            ),
        },
        "weight_sensitivity": {
