...
try {
  ...
  const tableSecurity = await rows(`
    select c.relname, c.relrowsecurity,
      has_table_privilege('service_role', format('%I.%I', n.nspname, c.relname), 'select,insert,update') as service_write,
  ...
  for (const table of tableSecurity) {
    ...
    check(`service_role_write:${table.relname}`, table.service_write === true);
  ...
  `, [functions]);
  check("commercial_effect_functions_present", functionSecurity.length === functions.length, `${functionSecurity.length}/${functions.length}`);
