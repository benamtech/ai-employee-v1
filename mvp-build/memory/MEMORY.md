...
**Current branch note (2026-07-18):** the active integration branch is `employee-production-tuesday`, based on `research`, with draft PR `#23`; `main` remains untouched. The approved Phase 2 remediation plan/registry passed run `29638985374`. Lane 1's relationship/authorization checkpoint merged at `b37d479a70983fcb3e88942b1f36481a07a97d17`; its five-case PostgreSQL matrix is green, but full P0 closure, real Supabase, browser/channel isolation, and helper privilege review remain pending. Lane 3 PR `#26` has a green command/effect contract and an intentionally red missing-kernel boundary; one scheduler-order-dependent concurrency assertion must be corrected before migration implementation. Production Supabase still stops at `0031_public_estimator.sql`. No new runtime/provider/browser/commercial/production acceptance is claimed.

## Index — newest first

- [2026-07-18 — Standard remediation checkpoint: Lane 1 integrated, Lane 3 contract/red boundary](2026-07-18-standard-remediation-lane1-lane3-handoff.md) — Records the validated control layer, Lane 1 red→green integration sequence and remaining scope, Lane 3 contract-green/pre-implementation-red state, the scheduler-order harness correction, exact CI/artifact IDs, unchanged live-acceptance state, and the next concrete move.
