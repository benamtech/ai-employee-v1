...
- [2026-07-04 20:41 — NEXT SESSION: debug Hermes runtime, Manager, events, connectors](2026-07-04-2041-next-session-debug-runtime-manager-events-connectors.md) — Paste-ready prompt for a focused debugging session. Forensics from the Sage/Ferraro Gmail test show Hermes returned text-only replies with `tool_turns=0`; Supabase has no connector/approval/event rows and only `owner_chat_turn` tool invocations, so no Gmail OAuth UI could surface. Priorities: local observability command, fix healthcheck to call Hermes `/health`/`/v1/capabilities`, and add/verify a real connector-start surface.
- [2026-07-04 19:05 — First live test on the sibling-Docker Hermes stack](2026-07-04-1905-first-live-test-sibling-docker.md) — First real local end-to-end run against live Supabase + Manager/Web + sibling Hermes. Bootstrap/provisioning worked and wrote `infra/.local/state.json`, but the first `local:chat` failed with `runtime_unreachable` because the Hermes API server was binding to `127.0.0.1:8972` inside the container, so host-side `/health` requests to the published port reset. This is the runtime-reachability blocker to fix before re-running the browser/API proof.
- [2026-07-04 18:40 — Claude Code handoff: live-feedback testing loop](2026-07-04-1840-claude-code-live-feedback-testing-handoff.md) — Paste-ready Claude Code Opus 4.8 prompt for continuing from the current posture: the local stack is live-testable but not MVP-usable, bypass scripts stay useful, browser onboarding should be exercised with plausible varied contractor data, and real live feedback should drive the next fixes before claiming phase acceptance.
- [2026-07-04 18:38 — Runtime bind + Hermes setup skip + Phase 6 live proof](2026-07-04-1838-runtime-bind-wizard-skip-phase6-proof.md) — Fixed the real local runtime bind at the rendered Hermes profile layer, frontloaded Hermes model/provider setup into every provisioned employee profile, and proved a fresh sibling Docker employee exposes `/health` + `/v1/capabilities`. Chat now reaches the OpenAI-compatible provider and fails honestly on missing/funded API key instead of asking for `hermes model`; no `/v1/runs/{id}/events` runtime-accepted proof yet. Phase 6 live Supabase RLS + turn-claim `run_id` proof passed (2 files / 5 tests).
- [2026-07-04 15:10 — NEXT AGENT: close Phase 6 metering, wire Phase 7 instrumentation](2026-07-04-1510-next-agent-finish-phase-06-wire-phase-07.md) — Handoff for the metering-focused agent. Phase 6 foundation is effectively done (six ledgers + `run_id` threaded ingress→deliver→wake→runtime→drain; RLS on all six **proven live** 4/4 after `0016`) — close it by recording the live RLS + `run_id`-crosses-RPC proof. Then Phase 7: add the missing `withMeteredToolInvocation`/`withMeteredExternalCall`/`estimateCost` helpers, seed `meter_pricing_versions` (unused today), **wire the universal `/manager/tools/:name` route** (`server.ts:81`, thread `run_id` into `ToolContext`) as the metering chokepoint, capture model tokens in `orchestrator-model.ts`, add provider wrappers, and prove the single-`run_id` Gmail→Stripe→reminder chain. Grounded in `docs/metering-implementation-plan.md`. Best-effort telemetry; no raw payloads.
- [2026-07-04 14:55 — NEXT SESSION: full local E2E test on the sibling-Docker stack](2026-07-04-1455-next-session-local-e2e-testing.md) — Paste-ready prompt to bring up the VPS-faithful no-SMS local stack (live Supabase + local Manager/Web + Caddy + one sibling Dockerized Hermes per employee) and drive onboarding → provision → chat → Work Surface end to end via infra scripts + browser + exposed API. Exercises the Phase 5 surfaces (live SSE stream, MCP-UI iframe cards, batching digest) and — the priority — **captures the real Hermes `/v1/runs/{id}/events` SSE proof** to pin `hermes-client.ts` and move the runtime gate toward `runtime-accepted`. Fix the stale-docker-group shell first.
