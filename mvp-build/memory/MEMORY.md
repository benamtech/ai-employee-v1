...
**Current branch note (2026-07-18):** the current production-boundary implementation and AMTECH Standard extraction are on `employee-work`, based on `research`, with draft PR `#19` targeting `research`. Do not edit `main` directly. The production-boundary source and CI gate are green; live production/provider/browser proofs remain pending. `STANDARD.md` is proposed and unapproved, so Phase 2 enforcement has not started.
