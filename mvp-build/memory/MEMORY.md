...
**Current branch note (2026-07-19):** the active integration branch is `employee-production-tuesday`, based on `research`, with draft PR `#23`; `main` is not the integration shortcut. The production-boundary proof anchor remains `a80908e1157e728eb666130c04b0ec285da55102`, covering branch CI and blank-PostgreSQL-through-`0068` evidence for Lane 1, C3, S2/S5/S6/S7/S9, S10.1, generated Manager routing, and the Manager image. The current UI/runtime source adds deterministic operating primitives, fixture labs, quiet-`observe` correction, bounded owner-stream reauthorization, canonical login/dashboard alignment, product-shell browser coverage, a live role/runtime vector registry, a proposed Standard amendment, and the scoped UI/runtime CODEGRAPH overlay. The first widened compiled browser run found and retained a standalone-server readiness race; the workflow now waits for the real standalone candidate and requires a final exact-head rerun. No approved real-Supabase, live-runtime, provider, fixture-free browser/SMS/Review, commercial-reconciliation, shared/fractional employee, broad role-perspective, public-employee, deployment, or launch acceptance is claimed.
