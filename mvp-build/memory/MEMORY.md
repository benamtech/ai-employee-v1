...
**Current branch note (2026-07-18):** `employee-work` remains based on `research`, with draft PR `#19` targeting `research`; do not edit `main` directly. The approved Phase 2 audit found 4 P0, 13 P1, 6 P2, 4 P3, and 2 P4 findings. The assignment-focused commits after `571ab08bbf7c84071ee97da1a71d89027618597f` moved into implementation before a complete causal model and behavioral test harness existed. Treat migrations `0039`/`0040` and related credential changes as an unvalidated architecture spike, not accepted remediation. Do not apply, deploy, merge, or claim them. No current-head CI, database, RLS, concurrency, provider, browser, SMS, commercial, or destructive validation has run. `SYSTEMIC-ROOT-CAUSE-ANALYSIS.md` now maps all 29 findings to seven systemic causes and makes a failing behavioral harness the next code step.

## Index — newest first

- [2026-07-18 — systemic root-cause reanalysis](2026-07-18-systemic-root-cause-reanalysis.md) — Corrected the premature assignment-only implementation sequence, quarantined the unvalidated `0039`/`0040` spike, classified all P0-P4 findings by issue type, mapped seven systemic causes and their causal links, added research applicability/unknown-space vectors, and changed the next code step to failing behavioral tests against the audited implementation.
- [2026-07-18 — assignment relationship P0 remediation pass](2026-07-18-assignment-relationship-p0-remediation-pass.md) — Historical record of the now-quarantined architecture spike. It added relationship graph migrations, assignment-aware source changes, and source-shape tests before sufficient causal analysis or behavioral validation. Do not treat as remediation evidence.
