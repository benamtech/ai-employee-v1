...
**Current branch note (2026-07-17):** the current production-boundary implementation pass is on `employee-work`, based on `research`, with draft PR `#19` targeting `research`. Do not edit `main` directly. The source and production-boundary CI gate are green; live production/provider/browser proofs remain pending.
