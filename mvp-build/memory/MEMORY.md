...
**Current branch note (2026-07-17):** the current production-boundary implementation pass is on `employee-work`, based on `research`, with draft PR `#19` targeting `research`. Closed PR `#18` is superseded. Do not edit `main` directly. The earlier `prod-ux` branch work has already been merged and is historical context, not the current branch instruction.

## Index — newest first

- [2026-07-17 — employee-work production boundary reconciler pass](2026-07-17-employee-work-production-boundary-reconciler-pass.md) — Bound the private Model Gateway to employee credentials/routes, made initial provisioning reconciler-owned, added leased provisioning/ambient workers with retries, compensation, drift repair, dead letters and replay, converged lifecycle/provider ingress, and added type/unit/image checks. Source-wired only: live Supabase still stops at migration `0031`; the first CI runs exposed and then drove fixes for Manager contract errors; no runtime/provider/browser acceptance is claimed.
