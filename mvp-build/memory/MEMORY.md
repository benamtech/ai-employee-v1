...
- [2026-07-05 01:40 — Tool availability bug + materialization direction](2026-07-05-0140-tool-availability-and-materialization-direction.md) — FACT: our profile `config.yaml` sets no `platform_toolsets.api_server`, so provisioned employees get Hermes's tiny default toolset (`terminal/file/web`) instead of the full shipped set (browser, code exec, image gen, cronjob, delegate, memory, kanban, skills, MCP client, ...); verify+fix via `platform_toolsets` + `GET /v1/toolsets`. `.env.tpl`'s "config.yaml is ignored" note is stale for the api_server path. PROPOSED direction (pending founder sign-off): two-tier materialization — universal schema-driven `ToolActivityDescriptor` renderer as the floor (any tool, no bespoke code) + MCP-UI/MCP Apps sandboxed-iframe widgets as the rich/extensible ceiling + native cards for Stripe/Gmail, all degrading to SMS/voice from one descriptor (multi-surface is our edge, MCP-UI is web-only). Parks the hermes-ui / agentchat-hermes / hermes-tag repos to revisit.
- [2026-07-05 01:01 — Connector actions as event-surface work + local observability](2026-07-05-0101-connector-actions-event-surface-and-observability.md) — Fixed the "Connecting Gmail now..." hallucination: connector lifecycle is now a typed `external_system_action` work event (`connector-events.ts`) delivered via the internal door, so pending/connected render on web card + SMS from one descriptor (general across connectors). Added the missing owner-visible Connect path (web `connector/start` route + Work Surface button) and employee awareness (`ownerTurnSystemPrompt` threaded into every owner turn + `connect_email` documented in the template). Built `npm run local:inspect` (warns "claimed connector work but no connect_email invocation"), fixed `healthcheck.mjs` to hit Hermes `/health`+`/v1/capabilities` with the sealed bearer, `onboard.mjs` writes `infra/.local/state.json`, env-gated Playwright connector spec. MCP native path confirmed supported by Hermes but deferred (needs live runtime to prove). Also corrected the stale "mvp-build is local-only, never push" git note (repo has a real origin now). Local gates green: typecheck, 39 files / 233 tests, lint, build.
- [2026-07-03 23:52 — Hermes alignment corrections (source-verified)](2026-07-03-2352-hermes-alignment-corrections.md) — Fixed four capability/response-shape mismatches in `hermes-client.ts` written against guessed field names, now verified against the real `NousResearch/hermes-agent` API: string-valued `session_key_header` detection (A1 — the header was silently never sent), `message.content` object extraction (A2), `run_submission`/`run_status` flag names (A3), nested `session.id` on session-create (A4), plus shared-type unions, real-shape test fixtures + new cases, and RUNBOOK flag-set corrections. Local gates green: typecheck, 38 unit files / 228 tests, integration 10 skipped clean, lint, build.
