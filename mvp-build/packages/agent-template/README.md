...
This is the **template** (`product-ai-employee-context.md` — "the product's soul"). Only the data differs per client; the soul is constant. Phase 1's `provision_employee` renders these files into a per-client Hermes profile + workspace, substituting double-brace token placeholders from the onboarding manifest.
...
**Render contract:** unknown or empty tokens are left **visible on purpose** as `DOUBLE_BRACE_TOKEN` placeholders so a bad render is obvious, not silent. The seven raw onboarding answers map to tokens deterministically (no AI step required); the verbatim transcript also seeds `workspace/brain/`.
