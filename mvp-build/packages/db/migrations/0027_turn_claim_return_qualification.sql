-- AMTECH AI Employee MVP — turn-claim RETURNING qualification fix
--
-- 0024 fixed the employee-turn lock insert race, but its
-- `returning employee_id into v_locked_employee` is ambiguous in plpgsql because
-- `employee_id` is also an output column in the RETURNS TABLE signature. Fresh
-- databases get the corrected 0024; this migration replaces already-applied
-- functions in existing environments.
