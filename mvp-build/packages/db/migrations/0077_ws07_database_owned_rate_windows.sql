...
-- WS-07 hardening: the database, not an internal caller, owns the shared
-- minute window. The compatibility parameter remains in the signature so
-- existing Manager deployments can roll forward without a coordinated cutover,
-- but it cannot select or shard rate authority.
...
begin
  ...
  if v_existing.request_key is not null then
    -- Only deterministic request/effect identity participates in replay
    -- conflict detection. Reservation estimates, correlation metadata, and the
    -- deprecated caller window cannot mutate the already-authoritative row.
  ...
  update model_gateway_rate_windows
   ...
   where credential_id = p_credential_id and rate_window_key = v_rate_window_key
