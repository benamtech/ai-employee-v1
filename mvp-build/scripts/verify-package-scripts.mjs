...
for (const pattern of workspacePatterns) {
  ...
  for (const entry of await readdir(parent, { withFileTypes: true })) {
    ...
    try {
      const child = JSON.parse(await readFile(join(parent, entry.name, 'package.json'), 'utf8'));
      workspacePackages.push({ path: `${pattern.slice(0, -2)}/${entry.name}`, name: child.name, build: child.scripts?.build });
    } catch (error) {
      errors.push(`invalid or missing workspace package.json: ${pattern.slice(0, -2)}/${entry.name}`);
    }
  }
}
...
if (errors.length) {
  console.error(JSON.stringify({ ok: false, errors, workspacePackages }, null, 2));
  process.exit(1);
}
console.log(JSON.stringify({ ok: true, workspacePackages }, null, 2));
