...
if (process.argv.includes("--help") || process.argv.includes("-h")) {
  console.log("Usage: pnpm audit [--strict]\nAudits package scripts, workflows, shell/Node/Python scripts, Docker/deploy config, environment handling, local fidelity, idempotency, attack vectors, and performance instrumentation. No repository code is executed.");
  process.exit(0);
}
...
const absolutePath = /(?:\/home\/[\w.-]+|\/Users\/[\w.-]+|[A-Za-z]:\\Users\\[^\\\s]+)/g;
...
for (const path of files) {
  ...
  const item = {
    ...
    dependency_graph: [...new Set([...text.matchAll(/(?:spawnSync|spawn|execFileSync?)\(["']([^"']+)/g)].map((m) => m[1]))],
  ...
  const matches = [
    ...
    [dockerAuthority.test(text), "host_authority"],
