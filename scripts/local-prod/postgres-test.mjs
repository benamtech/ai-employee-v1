...
if (process.argv.includes("--help") || process.argv.includes("-h")) {
  console.log("Usage: node scripts/local-prod/postgres-test.mjs\nRuns blank PostgreSQL 17 migrations and all repository integration tests using an already-present local image. It never pulls an image.");
  process.exit(0);
}
...
try {
  ...
  run("migrations", "npm", ["--prefix", "mvp-build", "run", "db:migrate"], { DATABASE_URL: url, STAGING_DATABASE_URL: url });
  run("integration_tests", "npm", ["--prefix", "mvp-build", "run", "test:integration"], { DATABASE_URL: url, STAGING_DATABASE_URL: url, LOCAL_PROD_OFFLINE: "1" });
  const { latest } = writeProof("postgres-tests", { status: "pass", image, steps });
  console.log(`proof_json:${latest}`);
} catch (error) {
  writeProof("postgres-tests", { status: "fail", image, steps, error: String(error?.message ?? error) });
