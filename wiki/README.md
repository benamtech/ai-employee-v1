...
> Sell first to **owner-operated residential painting & landscaping/hardscaping contractors**. Lead with a real estimate/work loop inside the **onboarding + consulting-sales surface**, then climb the ladder: **consultative first work → tuned Estimate package → live AI Employee → managed office loop.** The current reconciled MVP build plan is [`MVP/build-plan-current/`](MVP/build-plan-current/), the original whole-product packet is [`MVP/old-build-plan/`](MVP/old-build-plan/), and the current implementation is [`../mvp-build/`](../mvp-build/). Bookkeepers are sequenced beachhead #2 on the same platform.
...
- [`MVP/second-half-current-and-future-state.md`](MVP/second-half-current-and-future-state.md) - ⭐ **current second-half application state**: the backend is ahead of the owner product; the forward state is a Hermes-backed small-business materialization layer across web, SMS, signed previews, admin, and optional desktop/Deno clients. Companion to [`../mvp-build/second-half-plan/`](../mvp-build/second-half-plan/) - `active`
