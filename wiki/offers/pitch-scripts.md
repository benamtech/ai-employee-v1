# Pitch Scripts — Paint/Landscape Contractors

**Status: complete** · _Revised 2026-06-29 for the current motion: the demo is the live onboarding surface, you provision the Employee early, and the $300 tuned package is paid discovery — not the product. The condensed version lives in [`../00-decision.md`](../00-decision.md) §8–9; offer detail in [`wedge-offers.md`](wedge-offers.md) and [`../gtm/consulting-sales-surface.md`](../gtm/consulting-sales-surface.md)._

## Golden rule

Lead with the **result and a dollar number**; hold the tech until the demo. The seller's failure mode is retreating into explaining the AI under pressure. Every script below keeps you on the contractor's estimate pain, not on Hermes.

## Cold call opener (no-website / phone-only list)

"Hey, is this [name]? I'll be quick — I work with painters and landscapers, and I set up a textable employee that writes your estimates for you, in your own pricing and format, off the photos and messages from a job — and it asks you before anything goes to the customer. I'm not selling software — I'd just set it up on one real job you pick, no charge, so you see it. Worth two minutes?"

If "what is it / how": go to **Redirect 1**. If "I'm busy / on a job": "Totally — that's the point, you're slammed. When are you doing estimates, early morning or evenings? I'll call back then." (Contractors answer money calls; book the callback at their estimate time.)

## Email opener (with-website list)

> Subject: writing your estimates for you
>
> [Name] — saw [company]. Quick one: I set up textable AI employees for painters/landscapers that write the estimate from the job photos and the customer's messages — in your pricing and your format, usually within ~5%, and nothing sends without your okay. I'll set it up on one of your recent jobs for free so you can see the whole loop. Reply with a job and I'll send back the estimate.

## Discovery questions (keep it about their day)

- "How many estimates are you turning around a week right now?"
- "Are you writing them in the evening after the jobs, or during the day?"
- "What does a bid usually look like — back of a notepad, a spreadsheet, software?"
- "If I could write the next one *for* you off a voice memo, which job would be the best test?"

## The demo (this is the close) — run the real onboarding surface

1. Use the contractor's **own** recent job (photos + the customer's messages). Never a generic example.
2. In the live surface, produce a **line-item** estimate artifact — scope, quantity, unit price, the assumption behind each — with the signed link and the **approval card** visible. He's looking at his employee's actual output, not a slide.
3. Ask: **"Is that within a few percent of what you'd have charged?"** Let him check the lines.
4. On yes, set up his Employee on the spot (it provisions free-by-default) and sell the tuning: "That's *before* I tune it to your exact pricing. For $300 I load your markup, labor rates, and materials so it becomes your employee's pricing brain — dialed in, back to you in 48 hours."

## Tuned-package close ($300, paid discovery)

"Here's how it works: 20 minutes on the phone, you tell me your pricing the way you'd tell a new estimator — markup, crew day rate, material costs, your standard line items. I load it into your employee, send it back in two days, and from then on it writes your estimates within about 5%. Three hundred bucks, paid up front. Want the link?" _(Internally this is paid discovery that installs the business brain and improves the reusable `contractor_estimator` package — not a one-off file.)_

## Employee — provision early, then deepen as the loop is felt

Provision the Employee during/after the first work, not "someday." The trigger to climb is the felt pain:

"You've got the employee now — next step is it just *does* the whole thing: you text a voice memo from the truck, it pulls the customer's email thread, writes the estimate, sends you the link to approve, sends it through your email, then watches for the reply and asks if you want to send the deposit invoice. Same accuracy, you stop typing context. It's got its own number, knows your whole pricing, and gets better on its own as the AI improves. $750 to set up, then $1,000 a month."

Pro upsell (later): "Want it to also pull from your past estimates and photos automatically, draft the deposit invoice, and keep the job folder and follow-ups? That's the next tier — $1,500 a month." Each lane is a [deliverable type](../principle-deliverable-driven-surfaces.md) the same employee renders the same trusted way.

## Three panic redirects

**"How does it work / what AI is this?"**
"Honestly the tech matters less than the result. You pick a real job, I feed it what you already have, and it writes your estimate. You see every line and approve it before anything goes out. If it's not as good as yours, you don't pay."

**"Is this just ChatGPT?"**
"No. ChatGPT is a blank box you'd have to feed and babysit every time. This already knows your pricing, your markup, your materials, your format — so it writes *your* estimate, not a generic one. And it gets better on its own as the AI improves, without you doing anything."

**"That's too expensive."**
"An office person to do this is three to five grand a month. The tuning is three hundred bucks once — and if it wins you one extra job a month, it's paid for the year. If estimates aren't eating your evenings, don't buy it." Then stop talking.

## Objection: "I don't trust AI to price my jobs"

"Good — you shouldn't hand the number to anyone, software or a new hire. That's why every estimate comes back to you as line items with the assumptions shown, and nothing goes to a customer until you tap approve. It does the typing; you keep the pricing call."

## Beachhead #2 opener — bookkeepers (don't use until the skill is built)

"I help small bookkeeping and tax practices turn the pile of client documents into a sorted, missing-items checklist and a follow-up draft — it doesn't make accounting decisions, it just kills the sorting and chasing so your paid staff aren't doing it by hand."

## Demo-to-close checklist

1. Their real job, their inputs, in the live onboarding surface.
2. Line-item estimate artifact + approval card, same day or live.
3. "Within a few percent?" — let them verify.
4. Provision the Employee; "$300 to tune it to your pricing, link now, back in 48 hours."
5. As they feel each loop: climb to Gmail send/reply, deposit invoice, follow-up, Pro lanes.
