...
**UI/market update, 2026-07-14:** the owner-facing architecture should now be read through the AI-native Work Surface lens in [`MVP/ai-native-work-surface-research.md`](MVP/ai-native-work-surface-research.md). Plane 2 is not "webchat plus dashboards." It is the operating layer where chat creates or updates typed work, events become an office inbox, task-specific surfaces appear from work objects, approvals act like business permissions, and proof is part of the object. The public contractor estimator in [`gtm/free-estimator-funnel.md`](gtm/free-estimator-funnel.md) is the acquisition version of that architecture: an anonymous prospect experiences the same loop in one constrained task before any account exists.
...
5. **Chat-native operating surface.** Natural language is the primary input, but typed work objects are the product state. The owner should be able to say "I walked this job," then see the estimate, assumptions, approval, send action, customer reply, invoice, and proof as one work loop rather than hunting across chat, tasks, outputs, and activity.
