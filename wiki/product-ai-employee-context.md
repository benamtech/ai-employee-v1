...
The current product implication is larger than "an agent you can chat with." The employee plus Work Surface is an **AI-native operating environment for small-business work**: chat is the command language, business events become the inbox, estimates/invoices/messages become typed work objects, approvals become business permissions, and proof returns to the same object. The canonical UI research note is [`MVP/ai-native-work-surface-research.md`](MVP/ai-native-work-surface-research.md). The contractor free-estimator funnel is the public, narrow version of the same loop: one real job -> chat intake -> estimate object -> assumptions/download -> feedback -> free trial Employee that remembers the business.
...
AMTECH does not rebuild Hermes — the agent runtime, gateways, API server, MCP, cron, and skill loading are all Hermes. But "a thin factory that renders a template" **understates what AMTECH has actually built** as of mid-2026. The AMTECH layer is now a substantial **Manager control plane** plus the template: schema-first Manager tools exposed to the employee over per-employee scoped MCP credentials; provider connectors (Gmail, Stripe, and QuickBooks Online accounting) with custody, approval gates, idempotency, and proof; a tool-agnostic **materialization/capability layer** (`SurfaceEnvelope`/`WorkResource`/`CapabilityGraphNode`) that renders every tool result generically; owner surfaces (web employee desk, SMS ambient inbox, signed mobile Review, a tool-agnostic Connector Center, and a resurfacing projection); MCP-UI generative cards; an operator admin/metering layer; and the event spine (generic ingress, channel/session/presence routing, triage/batching). The template + factory is the *soul*; the Manager control plane is the *product-safe layer* that makes a frontier agent trustworthy for a non-technical owner. See [`MVP/second-half-current-and-future-state.md`](MVP/second-half-current-and-future-state.md) for the current build state.

The security correction still holds: a Hermes profile is a state/config boundary, not a filesystem sandbox by itself. On `terminal.backend: local` the agent runs with the host user's access; production containment comes from Docker/SSH/VM-style backends plus `terminal.home_mode: profile` [S085, S087]. The current build makes this a policy, not a hope — `docker` is the required production runtime backend and `local` is default-denied and hard-vetoed under production (see `mvp-build/apps/manager/src/lib/runtime-backend.ts`).
...
## What Hermes already does out of the box (the real product ceiling)

The wiki has historically framed the Employee around the Estimate skill. That undersells the substrate badly. Hermes is a **self-improving agent runtime, not a chatbot** — a 55-year-old painter's Employee is standing on a very capable base the day it's provisioned. Native, out of the box (grounded in `mvp-build/docs/hermes-agent-authoritative-record.md` and the Hermes docs [S086–S088]):

- **Real computer-operation tools, not just text.** Terminal/shell, filesystem, web fetch + search, a full browser (Browserbase), vision, image generation, and text-to-speech/voice, plus an MCP client to reach any external tool server. The employee can *do* work on a computer — research, produce documents, operate connected systems — not merely answer questions.
- **Skills it loads and grows.** It runs installed skills and can **author and improve its own skills** as it works, so each Employee gets better at *its specific business* over time — the compounding that makes early install valuable.
- **Durable memory + self-search.** Long-term memory scoped per employee, and full-text search over its own session history, so context accumulates instead of being re-pasted.
- **Autonomy primitives.** Turn-atomic Runs/Sessions with streaming, cancellation, and approval; cron/Jobs for scheduled autonomous work; and subagent **delegation** (`delegate_task`) so one Employee can spin up helpers for a bigger job.
- **Model-agnostic.** Multiple frontier providers swap underneath and multiple messaging gateways attach, so AMTECH absorbs model drift *for* the owner.

The implication for positioning: the product's true ceiling is **"an employee that can help start or run a small business"** — website work, research, purchasing, invoices, follow-ups, marketing, records, scheduling, and connected-system tasks — **not** "an estimate generator." The Estimate skill is the **wedge and first proof object** because it is the most universal, frequent, money-deciding task a contractor has; the free public estimator makes that proof available before signup, but it is deliberately *not* the ceiling. (See the second-half plan's product thesis in [`MVP/second-half-current-and-future-state.md`](MVP/second-half-current-and-future-state.md) and the funnel in [`gtm/free-estimator-funnel.md`](gtm/free-estimator-funnel.md).)

AMTECH's job is therefore **not** to build capability — Hermes already has it — but to make that native capability **safe** (the confirmation gate on anything that leaves the business or spends money), **legible** (the materialization/capability layer and owner-language surfaces), and **operable for a non-technical owner** (the Manager control plane, connector custody, provisioning, and the fleet operations still being finished). The moat is the gap between "a frontier agent can do this" and "a painter has it reliably running his office," not the raw capability.
...
- **Contain the shell.** Each client's tool execution must be sandboxed. In the current build `docker` is the **required** production runtime backend, and `local` (no container isolation) is default-denied and hard-vetoed under production — `local` is a dev/demo escape hatch only. `terminal.home_mode: profile` separates CLI home/credentials, but does not by itself prevent filesystem access on the local backend [S087]. (The container *isolation policy* is enforced; the production container *launch/supervision mechanism* is a known open gap — see the production deploy readiness review under `mvp-build/docs/`.)
...
| Employee · + Accounting | + QuickBooks Online | Drafts expenses/bills/invoices/payments into QuickBooks behind the confirmation gate; pulls P&L / balance-sheet / AR / AP; resolves customer/vendor names with disambiguation, never guessing — the bridge to bookkeeper-adjacent work (beachhead #2) on the same platform |
