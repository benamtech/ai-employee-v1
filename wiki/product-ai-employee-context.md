...
The implication for positioning: the product's true ceiling is **"an employee that can help start or run a small business"** — website work, research, purchasing, invoices, follow-ups, marketing, records, scheduling, and connected-system tasks — **not** "an estimate generator." The Estimate skill is the **wedge and first proof object** because it is the most universal, frequent, money-deciding task a contractor has; it is deliberately *not* the ceiling. (See the second-half plan's product thesis in [`MVP/second-half-current-and-future-state.md`](MVP/second-half-current-and-future-state.md).)
